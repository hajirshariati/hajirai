### FILE 7: `app/routes/webhooks.app.uninstalled.jsx` (REPLACE ENTIRE FILE)
...
### FILE 8: `app/routes/app.knowledge.jsx` (REPLACE ENTIRE FILE)
...
1. **Run DB migration** (on laptop, with `DATABASE_URL` set):
   ```bash
   npx prisma migrate dev --name product_enrichment
   ```
...
3. **Test**: Upload a CSV with a `sku` column in the Knowledge Base admin. The UI should show "Linked X SKUs (Y matched your catalog)". Then ask the chat widget about a product — the AI should use `search_products` or `get_product_details` tools instead of guessing.

4. **Optional**: After confirming everything works, the standalone `hajirai-chat-server` Railway service can be retired since the Anthropic SDK + agentic loop now runs inside the main Remix app.

## Important Notes

- The `@anthropic-ai/sdk` package (^0.39.0) must be in `package.json` dependencies. If not already present, run `npm install @anthropic-ai/sdk` before deploying.
- The repo's existing `app/routes/chat.jsx` likely forwards to an external chat-server. This new version replaces that entirely — it calls the Anthropic API directly from within the Remix app.
- `getKnowledgeFilesWithContent` must exist in `app/models/ShopConfig.server.js`. If it doesn't, add it — it should query `prisma.knowledgeFile.findMany({ where: { shop }, select: { id: true, fileType: true, content: true } })`.
