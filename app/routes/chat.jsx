    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          const { runWithGroundingRetry } = await import("../lib/llm-owns-turn.server.js");
          // LLM-owns-turn is the only response engine now — the legacy dispatcher
          // cascade (policy / product-engine / resolver-no-match fall-through) was
          // removed 2026-06-24. Bare block preserves the per-turn local scoping the
