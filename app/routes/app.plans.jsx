...
export default function PlansPage() {
  ...
  return (
    <Page>
      ...
      <Layout>
        {actionData?.message ? (<Layout.Section><Banner tone={actionData.ok ? "success" : "critical"}>{actionData.message}</Banner></Layout.Section>) : null}
        <Layout.Section>
          <Card>
            <BlockStack gap="300">
              <InlineStack align="space-between" blockAlign="center">
                <BlockStack gap="100">
                  <Text as="h2" variant="headingMd">Current plan: {current.name}</Text>
                  <Text as="p" tone="subdued">{limit === Infinity ? `${used.toLocaleString()} conversations this month (unlimited)` : `${used.toLocaleString()} of ${limit.toLocaleString()} conversations used this month`}</Text>
                </BlockStack>
                <Badge tone={currentPlanId === "free" ? "attention" : "success"}>{current.price === 0 ? "Free" : `$${current.price}/mo`}</Badge>
              </InlineStack>
                  ...
                  <Card>
                    <BlockStack gap="300">
                      <BlockStack gap="100">
                        ...
                        <Text as="p" variant="heading2xl">{plan.price === 0 ? "Free" : `$${plan.price}`}{plan.price > 0 ? <Text as="span" variant="bodyMd" tone="subdued"> / month</Text> : null}</Text>
                        <Text as="p" tone="subdued">{formatLimit(plan.conversationsPerMonth)} conversations/mo</Text>
                      </BlockStack>
                      <List type="bullet">{plan.features.map((f) => <List.Item key={f}>{f}</List.Item>)}</List>
                      <Form method="post" onSubmit={() => setPendingPlan(id)}>
                        ...
                        <Button submit fullWidth variant={isCurrent ? "secondary" : "primary"} disabled={isCurrent || submitting} loading={submitting && pendingPlan === id}>
        ...
        <Layout.Section>
          <Card><BlockStack gap="200">
            ...
            <Text as="p" tone="subdued">Charges are handled by Shopify and appear on your Shopify invoice. You can cancel or change plans at any time. Unused conversations don't roll over.</Text>
          </BlockStack></Card>
        </Layout.Section>
      </Layout>
    </Page>
