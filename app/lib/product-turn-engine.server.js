...
const ACCOUNT_QUESTION_RE =
  /\b(?:how\s+many\s+points|how\s+much\s+(?:credit|store\s+credit|in\s+rewards?)|points?\s+(?:balance|do\s+i\s+have|i\s+have|i'?ve\s+got)|(?:my|the)\s+(?:points?|balance|rewards?|tier|loyalty|account|profile|store\s+credit)|loyalty\s+(?:points?|status|balance|tier|account)|vip\s+(?:status|tier|perks?)|redeem(?:able)?\s+(?:rewards?|points?)|where\s+is\s+my\s+order|track\s+(?:my\s+)?(?:order|package|shipment)|order\s+(?:status|number|tracking|history|details)|my\s+(?:last\s+|recent\s+|previous\s+)?order|recent\s+orders?|past\s+(?:order|purchase)s?|order\s+history|referral\s+(?:link|page|program|code|url)|my\s+referral|refer\s+(?:a\s+)?friend|give\s+\$?\d+\s+get|share\s+(?:my\s+)?(?:link|referral)|cancel(?:lation)?\s+(?:my|the|an?|this|that|recent|last|previous)?\s*(?:order|purchase|item)?|i\s+(?:want|need|would\s+like)\s+to\s+(?:cancel|refund)|refund\s+(?:my|the|an?|this|that)\s+(?:order|purchase|payment)|(?:damaged|broken|wrong|missing)\s+(?:item|order|product|package|shipment)|(?:billing|charge|charged|payment)\s+(?:issue|problem|wrong|error))\b/i;
