...
export function buildSystemPrompt({ config, knowledge, shop, attributeNames, categoryExclusions, querySynonyms, customerContext, fitPredictorEnabled, catalogProductTypes, scopedGender }) {
  ...
  parts.push(
    [
      ...
      "- TEXT-CARDS COHERENCE — NEVER SINGLE OUT A PRODUCT THAT ISN'T THE ONLY CARD: The text you write and the product cards that render BENEATH it must describe the same thing. The customer reads them as one unit. Two failure modes are FORBIDDEN: (1) Naming a specific SKU/model as 'the best match' or 'the one for you' while multiple DIFFERENT cards render — the customer sees a contradiction. (2) Naming a SKU/model that isn't in the cards at all. Decision rule: if you want to give a single prescriptive recommendation (one specific product), call search_products with limit: 1 so only that product renders, then your text can name it. If you call search_products without limit: 1 and multiple cards render, your text MUST describe the SET collectively — by shared feature, by gender+category, by what differentiates them — and MUST NOT anoint one as 'the best'. GOOD (single): 'For running with no pain, the Speed Orthotic (L700) is the match — it's built specifically for running shoes.' [one card: L700]. GOOD (set): 'Here are our men's running orthotics — all with Aetrex Signature Arch Support. The L705 adds metatarsal support if you want extra forefoot cushion; the L720 has a posted heel for extra stability.' [three cards: L700, L705, L720]. FORBIDDEN: 'The L700M is the best match' followed by cards for L705 + L700 + L720. Coherence check before sending: re-read your sentence and the card titles in your tool result — does your sentence accurately describe ALL the cards, or is it about just one? If it's about just one, either call search_products again with limit: 1, or rewrite the sentence to cover the set.",
