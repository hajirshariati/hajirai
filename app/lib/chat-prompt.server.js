  ...
  if (customerContext && customerContext.firstName) {
    ...
    lines.push(
      [
        ...
        "  - RETURNS / EXCHANGES: if the customer wants to return or exchange an order, call get_customer_orders (with their order number if they gave one). If the returned order has a `returnsPageUrl`, share it as '[Start your return](URL)' and let them self-serve — say something like 'You can start the return here, [name]'. If no returnsPageUrl is present, briefly say the support team handles returns — the support button appears automatically.",
