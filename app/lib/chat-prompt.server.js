...
export function buildSystemPrompt({ config, knowledge, retrievedChunks, shop, attributeNames, categoryExclusions, querySynonyms, customerContext, fitPredictorEnabled, catalogProductTypes, fullCatalogProductTypes, scopedGender, answeredChoices, categoryGenderMap, activeCampaigns, merchantGroups }) {
  ...
  parts.push(
    [
      ...
      "- PRODUCT-NAME LOOKUP DISCIPLINE — TOOL FIRST, NEVER NARRATE THE LOOKUP: When the customer names a specific product by name (e.g. 'Vania', 'Tamara', 'Danika', 'Carly', 'Jillian', or any proper product name), comparing them, asking about color/availability, or asking 'show me X' / 'how about X' / 'do you have X' — your FIRST action this turn MUST be a tool call. NOT a sentence. NOT 'Let me pull up X' or 'Let me look up X' or 'Let me get the details' — those phrases get stripped by the server's banned-narration guard and leave you with an empty response. Just call the tool: `search_products` with the product name as the query string (e.g. `{ query: 'Vania' }`), or `lookup_sku` if you have a SKU/handle, or `find_similar_products` for 'similar to' / 'same support as' style requests. After the tool returns, write your answer using the actual data. If the tool returns nothing matching the named product, say honestly: 'I can't find a product called <name> in our catalog right now — could you double-check the spelling, or share the link to the product page?' Production scenarios this rule prevents: customer asks 'what other shoes have same support as danika' → AI writes 'Let me pull up the Danika details!' → server strips it → empty bubble shipped. AI asks 'Can you show me the Vania or Tamara in red?' → AI writes nothing useful → empty bubble. The strip is correct; YOUR job is to call the tool instead of announcing it.",
