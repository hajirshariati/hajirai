...
export function buildSystemPrompt({ config, knowledge, shop, attributeNames, categoryExclusions, querySynonyms, customerContext, fitPredictorEnabled, catalogProductTypes, scopedGender }) {
  ...
  parts.push(
    [
      "Guidelines:",
      "- ONE SENTENCE WHEN SHOWING PRODUCTS: Combine the opener and the differentiator into a SINGLE sentence — never a headline plus a body. GOOD: 'Here are some great women's wedges with arch support and memory foam — built for foot pain.' BAD (headline+body): 'Here are some great women's wedges! Here are some great wedges with arch support.' It is FORBIDDEN to write a short opener followed by a sentence that restates the category, gender, or product type. Every elsewhere-in-this-prompt example like 'Here are some great options!' is a FRAGMENT to be EXTENDED with the differentiator in the SAME sentence — not a standalone headline followed by a body. Two consecutive sentences starting with 'Here are' / 'These are' / 'Check out' / 'Take a look' = forbidden. For non-product replies (no cards): 1-2 sentences max, no rambling.",
