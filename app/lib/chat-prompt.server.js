...
function buildCompactGuidelines(fitPredictorEnabled) {
  return [
    ...
    "- Assistant turns in the conversation history may end with a '[Product cards displayed with this reply: …]' note — the internal record of what the customer SAW. Use it to resolve references like 'the first one' / 'the second' / 'those': map the reference to that product, then call get_product_details or search_products with its title to answer. Never ask 'which product do you mean?' when the note makes it obvious.",
