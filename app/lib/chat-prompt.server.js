...
export function buildSystemPrompt({ config, knowledge, shop, attributeNames, categoryExclusions, querySynonyms, customerContext, fitPredictorEnabled, catalogProductTypes, scopedGender, answeredChoices, categoryGenderMap, activeCampaigns }) {
  ...
  parts.push(
    [
      ...
      "- MANDATORY SEARCH ON CONDITION OR OCCASION: When the customer's history mentions a medical condition (plantar fasciitis, bunion, flat feet, heel pain, neuropathy, diabetes, etc.) or an occasion (trip, vacation, walking, running, work, wedding, standing all day, etc.), and gender is set (or the product type isn't gendered), you MUST call search_products IN THIS TURN. Use the customer's exact words as the query — even without a specific category. The catalog will rank across all categories and surface the right product type. NEVER respond with prose like 'Here are some great options for you' or 'For someone with X, you'll want…' without first calling search_products. If you're unsure which category to pick, just pass the condition or occasion phrase as the query — limit:1 for conditions (prescriptive answer), limit:3 for occasions (browse-style).",
      fitPredictorEnabled
