...
export async function upsertAttributeMapping(shop, { attribute, sourceType, namespace, key, prefix }) {
  return prisma.attributeMapping.upsert({
    where: { shop_attribute: { shop, attribute } },
    update: { sourceType, namespace, key, prefix, updatedAt: new Date() },
    create: { shop, attribute, sourceType, namespace, key, prefix },
  });
}
...
export function buildMetafieldFragment(mappings) {
  const metafieldMappings = mappings.filter((m) => m.sourceType === "metafield" && m.namespace && m.key);
