...
// Parse a CSV and replace all enrichment rows sourced from the given file.
// Returns { matched, total, skuColumn } or { error } on failure.
export async function upsertEnrichmentsFromCsv(shop, file, content) {
  ...
  // Wipe previous rows sourced from this file so a replaced upload doesn't
  // leave stale SKUs lying around.
  if (sourceFileId) {
  ...
  // Count how many of the incoming SKUs actually exist in the synced catalog.
  // We still store enrichment for un-matched SKUs — products may sync later —
  // but the count is useful to show the merchant.
