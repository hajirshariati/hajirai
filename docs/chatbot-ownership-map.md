...
| Concern | Owner | Notes |
|---|---|---|
| **Workflow** (which kind of turn this is) | **TurnPlan** (`app/lib/turn-plan.server.js`, `planTurn`) | Classifies every turn into one of 13 workflows: `policy_account`, `customer_service`, `availability`, `prior_evidence_availability`, `comparison`, `named_product_advisory`, `condition_recommendation`, `multi_recommendation`, `compatibility`, `sale_browse`, `sizing_help`, `browse`, `clarification`. Also owns `searchRequired`, `clarificationAllowed`, `productDisplayPolicy`, `gender`. |
| **Latest-turn scope / state hygiene** | **Turn scope** (`app/lib/turn-scope.js`) | Runs right after the classifier in `chat.jsx`. Classifies each message as FOLLOW_UP (deictic / prior-card reference — "those", "either", "the first one", "what about…") or NEW_INDEPENDENT_ASK. On a new independent ask, `scopeAttributesToTurn` drops stale `category` / `color` / `condition` / `useCase` from `ctx.classifiedIntent.attributes` so planTurn, the over-elicitation guard, the recommender gate, and search all read THIS turn's facts — a prior "flat feet / black / sneaker" can never leak into "I'm going on vacation…". Also owns `isShortAmbiguousReply` ("not sure" / "maybe", ≤4 words): a content-free reply is only an answer to an ACTIVE pending question, never enough to infer a condition. |
| **Condition normalization** | **Condition normalize** (`app/lib/condition-normalize.js`) | Single source of truth mapping free-text foot-condition vocabulary → ONE canonical key (order-deterministic). `conditionFromText` guarantees "Morton's neuroma" → `mortons_neuroma` and never collapses into `plantar_fasciitis`; an ambiguous reply ("not sure") returns null (no condition inferred). The recommender enrichment (`recommender-tools.server.js`) consumes it so classifier, memory, and answer agree on the condition. |
| **Order / delivery / account ISSUES** | **Customer-service handoff** (`chat.jsx`, `workflow=customer_service`) | A problem needing a HUMAN (missing/late/wrong/damaged delivery, refund/return/exchange/cancel request, order lookup, payment, account access) → routed BEFORE policy/browse. No product search, no cards; the LLM writes one friendly acknowledgement and chat.jsx deterministically attaches the live-chat CTA (`supportHandoffCta` → `support_cta`, Zendesk/Intercom/Gorgias, Support Hub URL only as fallback). Informational policy questions ("what's your return policy?") stay `policy_account`. |
...
| **A new constraint applied to the SET just shown** | **Prior-evidence availability** (`chat.jsx` + `app/lib/prior-evidence.js`, `priorEvidencePinnedCards`) | For `workflow=prior_evidence_availability` ("do they come in black?" after a multi-product turn). Remaps EACH previously-displayed family to the new color/size/width via Availability Truth, shows only the matching prior products' cards, and OWNS the deterministic answer text ("Yes — Tamara and Danika come in black. I'm not seeing Mandy…"). Supports **multiple requested colors** in one turn ("champagne or rose") — each prior family is checked against every requested color and the answer names each honestly ("Savannah comes in Champagne, but I'm not seeing Rose"). Cards are a subset/remap of the prior families — **never the scorer, never random alternates**. |
| **Condition / advisory cards** | **Evidence selection** (`chat.jsx` + `app/lib/evidence-select.js`, `evidencePinnedCards`) | For `workflow=condition_recommendation`: picks 2-3 distinct-family cards from THIS turn's evidence pool — preferring the families the LLM named in its text — and pins them (`cardOwner=evidence-plan`). The scorer never hands 6 cards to the carousel; text and cards stay aligned. |
...
| **Sales voice (no process narration)** | **Sales-voice guard** (`app/lib/sales-voice.js`) | The reply must read like a store associate, never narrate retrieval (searches, tools, system/catalog/database, filters, "results", "what I'm seeing/getting"). `detectProcessNarration` is BLOCKING on sales-voiced workflows (condition/advisory/comparison/availability/prior_evidence/multi-with-cards) → rewrite-only retry; the final emit scrub removes only the offending sentence, falling back to a warm sales-safe line if that leaves a fragment. Sales-judgment turns route to the stronger model first. |
...
- Latest-turn scope + condition normalization (state hygiene):
  `scripts/eval-turn-scope.mjs` (PRD 2026-06-27 regressions A–F).
